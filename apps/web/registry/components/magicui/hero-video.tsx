...
export const HeroVideoAction = ({ children }: { children: React.ReactElement }) => {
  ...
  return (
    <div className="absolute left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 transform">{childWithOnClick}</div>
  )
}
...
const HeroVideo = ({ children, image, title = 'Demo' }: HeroVideoProps) => {
  ...
  return (
    <HeroVideoContext.Provider value={{ openModal, closeModal }}>
      <div className={cn('absolute inset-[1px] flex overflow-hidden rounded-2xl')}>
        {actionChild}
        <img className="pointer-events-none h-full w-full object-cover" src={image} />
      </div>
